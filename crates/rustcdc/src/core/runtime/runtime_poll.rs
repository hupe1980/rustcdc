...
impl CdcRuntime {
    ...
    pub async fn poll_event_batch(&mut self) -> Result<EventBatch> {
        ...
        // Every return, including an error. This answers one question — "is the poll loop
        // turning?" — so an idle source returning empty batches must keep it fresh, and a
        // source failing fast in a retry loop is not a *blocked* loop. Recording it only
        // where events are delivered would make a quiet database read as stalled.
        //
        // `last_delivery_at_ms`, written in `deliver_buffered_batch`, is the separate
        // "when did events last arrive" signal that separates `Healthy` from `Idle`.
