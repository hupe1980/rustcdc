...
fn main() {
    ...
    // The Markdown doctests read the published documentation from `site/`, which is a
    // repository-level asset one directory above this crate and therefore not part of the
    // published `.crate`. Declaring the cfg only when the files are actually there lets
    // the same source tree do both: in the repository the guides are compiled and run;
    // from a published crate the module simply does not exist, instead of failing to
    // expand `include_str!` on a path that cannot be there.
    //
    // `#[cfg(doctest)]` alone is not enough — cfg-stripping removes the module before
    // macro expansion, so `cargo build` and `cargo doc` are fine either way, but
    // `cargo test --doc` inside an unpacked `.crate` would not be.
    println!("cargo:rerun-if-changed=../../site/content/docs");
    if std::path::Path::new("../../site/content/docs/api.md").exists() {
        println!("cargo:rustc-cfg=rustcdc_markdown_doctests");
    }
