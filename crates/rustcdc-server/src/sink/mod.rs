...
// `SinkAdapter::delivery_metrics()` — four generic fields. Everything else a sink reports
// (HTTP status classes, batch-size and retry-delay histograms, pending bytes, Iceberg
// orphaned files, Kafka OAUTHBEARER token health) has no room there and is unreachable
// once a binding is inside the router. Reading metrics back *out* of the router therefore
// pins ~30 families at zero, which no test crossing only the erasure boundary can see.
//
// So a handle is taken on the way *in*, like `KafkaTransactionHandle`, and the binding
// publishes into it at each flush. Flush is the right cadence: the run loop samples before
// and after each batch and a batch ends with a flush, so both samples land on published
// values.
