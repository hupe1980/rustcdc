...
// Every source field the configuration reference documents with a default must load when
// omitted, and take that default. Expected values come from the library's `Default`, so
// the reference, the serde attribute and `Default` are pinned to one answer.
