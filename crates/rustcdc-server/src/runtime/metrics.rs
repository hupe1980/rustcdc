    ...
    pub(crate) fn render_prometheus(&self) -> String {
        format!(
            "{}{}{}",
            {
                ...
                // Infallible: the sink is an in-memory `Vec`.
                let _ = rustcdc::core::write_runtime_metrics_prometheus(
                    &self.runtime_admin,
                    &mut rendered,
                );
...
mod tests {
    ...
    fn runtime_admin_metrics_render_one_hot_health_and_data_loss_counter() {
        ...
        // The library's renderer — the only one. This crate used to have a second copy
        // that emitted a different name for the slot lag, omitted three families, and
        // carried no `source_type` label at all.
        let mut rendered = Vec::new();
        rustcdc::core::write_runtime_metrics_prometheus(&snapshot, &mut rendered)
            .expect("writing to a Vec is infallible");
        let out = String::from_utf8(rendered).expect("valid UTF-8");

        assert!(
            out.contains("rustcdc_runtime_health{source_type=\"postgres\",verdict=\"stalled\"} 1"),
            "{out}"
        );
        for quiet in ["healthy", "idle", "not_running"] {
            assert!(
                out.contains(&format!(
                    "rustcdc_runtime_health{{source_type=\"postgres\",verdict=\"{quiet}\"}} 0"
                )),
                "verdict {quiet} must render 0:\n{out}"
            );
        }
        assert!(
            out.contains("rustcdc_runtime_events_skipped_total"),
            "{out}"
        );
        // The canonical name. The duplicate renderer called this
        // `rustcdc_runtime_replication_slot_lag_bytes`, and the shipped alert rules
        // referenced both spellings — so one of the two could never fire, on the signal
        // whose unbounded growth fills a PostgreSQL primary's WAL volume.
        assert!(
            out.contains("rustcdc_replication_slot_lag_bytes{source_type=\"postgres\"} 42"),
            "{out}"
        );
        assert!(
            !out.contains("rustcdc_runtime_replication_slot_lag_bytes"),
            "the divergent spelling must not come back:\n{out}"
        );
