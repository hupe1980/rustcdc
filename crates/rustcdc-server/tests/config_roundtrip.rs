...
/// The configuration on the site's landing page must actually load.
///
/// It is the first thing a visitor reads and the page's claim is "a whole pipeline is one
/// file", so a sample that `validate-config` rejects is worse than no sample. Nothing
/// compiles a fenced TOML block, so without this it rots silently: the previous one
/// carried a `[source] type = "postgres"` key the schema does not have, a `[state]
/// backend = "kafka_topic"` shape that had moved, and an `[admin] enabled` field that no
/// longer exists — three schema generations behind, on the front page.
#[test]
fn the_landing_page_configuration_sample_loads() {
    let landing = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(2)
        .expect("workspace root")
        .join("site/content/_index.md");
    let markdown = std::fs::read_to_string(&landing).expect("read the landing page");

    let sample = markdown
        .split("```toml")
        .nth(1)
        .and_then(|rest| rest.split("```").next())
        .expect("the landing page must carry a TOML sample");

    // The sample references secrets by environment variable, which is the behaviour it is
    // demonstrating; supply them so the check is about the schema.
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("landing.toml");
    std::fs::write(&path, sample).expect("write the sample");

    // SAFETY: `set_var` is unsafe in Rust 2024. This is a single-threaded test process
    // setting variables the loader reads immediately below; the architecture allowlist
    // covers `test_env` for the same reason.
    #[allow(unsafe_code)]
    unsafe {
        std::env::set_var("POSTGRES_PASSWORD", "test-only-not-a-real-credential");
        std::env::set_var("PII_HMAC_KEY", "test-only-not-a-real-key");
    }

    rustcdc_server::config::load(&path).unwrap_or_else(|error| {
        panic!(
            "the landing page's configuration sample does not load: {error}\n\n\
             It is the first thing a reader sees. Fix the sample, not this test."
        )
    });
}
