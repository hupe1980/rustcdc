...
impl IncrementalSnapshotBackend for SqlServerSnapshotBackend {
    ...
    ///
    /// [`event_in_bracket`](crate::source::IncrementalSnapshotBackend::event_in_bracket) exists
    /// to be overridden because on a log-tailing connector the watermark can run *ahead* of what
    /// a chunk `SELECT` can see — as it does on PostgreSQL and MySQL. SQL Server inverts that:
    /// `fn_cdc_get_max_lsn()` reports what the capture job has already harvested into `cdc.*`,
    /// and the capture job reads the log asynchronously *after* transactions commit. The
    /// watermark therefore lags visibility rather than leading it.
    ///
    ...
    /// changes nothing downstream. So this backend keeps the default ordinal bracket
    /// deliberately, not for lack of a way to override it.
