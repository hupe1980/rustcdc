...
- **Do not re-type the values.** Every value is text, exactly as Snowflake rendered it. Parsing
  ...
  [the crate's value contract](@/library/api.md#column-values-are-text-on-every-connector-and-every-path)
  exists to keep.
...
Every field is in the [configuration reference](@/library/config-reference.md#snowflake-source-configuration).
