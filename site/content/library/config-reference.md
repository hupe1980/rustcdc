...
| `health_stall_threshold_ms` | `Option<u64>` | `None` (derived) | How long the poll loop may go without returning before [`HealthVerdict::Stalled`](@/library/runbook.md#health-verdict-idle-vs-stalled). `None` derives `max_poll_wait_ms × 6`, floor 30 s. |
...
Why, and what each measures at: [WAL transport](@/library/architecture.md#wal-transport) ·
...
| Value the source could not supply | *key absent* | Listed in `unavailable_columns`; see [partial payloads](@/library/api.md#partial-payloads-read-this-before-writing-a-sink). |
...
[Troubleshooting](@/library/troubleshooting.md).
...
through Snowflake Streams; [the Snowflake page](@/library/snowflake.md) has the reasoning, which
