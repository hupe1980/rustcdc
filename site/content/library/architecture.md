...
[on-demand snapshots](@/library/config-reference.md#on-demand-snapshots). Because such a table is not
...
[Incremental snapshot for a custom source](@/library/api.md#incremental-snapshot-for-a-custom-source).
...
- the peek is **non-consuming**: `pg_logical_slot_get_changes_guts` begins reading at the slot's
  ...
  [measured performance](@/library/reliability-testing.md#measured-performance) does not settle
- delivery latency is bounded by the poll interval rather than pushed by the server. **Note
  ...
  [measured performance](@/library/reliability-testing.md#measured-performance)
...
- `binlog_transaction_compression` is read transparently; events unpacked from a
  ...
  their own — see [the config reference](@/library/config-reference.md#binlog-transaction-compression)
...
- [API Guide](@/library/api.md)
- [Configuration Reference](@/library/config-reference.md)
- [Schema Evolution and DDL Capture](@/library/schema-evolution.md)
- [Reliability Testing Guide](@/library/reliability-testing.md)
- [Operator Runbook](@/library/runbook.md)
- [Troubleshooting Guide](@/library/troubleshooting.md)
