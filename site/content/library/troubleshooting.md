...
establish them first using the deployment guidance in [Deployment](@/library/deployment.md).
...
| WAL/binlog purged | See [Replication Slot Divergence Recovery](@/library/runbook.md#replication-slot-divergence-recovery) |
...
[safety invariants](@/library/architecture.md#safety-invariants) for the table.
...
| MySQL failover without GTID | `gtid_mode_enabled` is `false` and you promoted a replica | Enable [GTID positioning](@/library/config-reference.md#gtid-positioning). Binlog coordinates are server-local; nothing can make them survive a failover. |
...
| Event validation rule violated | Check the [API guide](@/library/api.md) and the `src/core/event.rs` validation contract; verify the source is generating events correctly |
