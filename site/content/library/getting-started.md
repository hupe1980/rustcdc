...
- A role with the **`REPLICATION`** attribute, and a **direct** connection to the server — the
  ...
  [`wal_transport`](@/library/config-reference.md#wal-transport) for the SQL-based fallback.
...
> [the runbook](@/library/runbook.md).
...
Alert on exactly that; see [health verdict](@/library/runbook.md#health-verdict-idle-vs-stalled)
...
- [Architecture](@/library/architecture.md) — the commit barrier and checkpoint model, and why
  the ack is a separate step
- [API guide](@/library/api.md) — transforms, codecs, schema registries, custom sinks
- [Configuration reference](@/library/config-reference.md) — every option and the failure it
  prevents, including the MySQL and SQL Server server-side prerequisites
- [Reliability testing](@/library/reliability-testing.md) — replay a captured stream inside your
  own test suite
- [Runbook](@/library/runbook.md) — what to alert on before this goes to production
