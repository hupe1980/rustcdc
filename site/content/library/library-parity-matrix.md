...
| Sources | PostgreSQL 14–18 | PostgreSQL, MySQL, MariaDB, SQL Server behind one `Source` trait, plus Snowflake as a **source** over the `CHANGES` clause |
...
| Built-in field mapping transform primitives (copy/rename/set/remove) | Covers common schema-alignment workloads without mandatory custom/WASM code | Implemented | src/transform/field_mapping.rs, src/transform/mod.rs, site/content/library/config-reference.md |
...
| `SqlPeek` fallback is 4–5× slower than the default transport | Environments that cannot grant a replication connection pay a measured 4–5× on capture time, and each poll re-reads WAL from the slot's `restart_lsn` rather than continuing from the last position | Inherent to `pg_logical_slot_peek_binary_changes` being non-consuming. Numbers and method: [measured performance](@/library/reliability-testing.md#measured-performance) |
...
| MySQL trades the connection pool for credential refresh | `auth_mode = AwsIamToken` disables pooling on MySQL: one freshly authenticated connection per request | `mysql_async::Opts` is immutable and the driver has no per-connection credential hook, so a pool authenticates every connection it ever opens with the password resolved when it was built — an RDS IAM token expires in 15 minutes. PostgreSQL re-resolves per connection and keeps its pooling. The handshake cost is affordable at a CDC connector's request rate, and applies only when the operator opts in. [Config reference](@/library/config-reference.md#aws-iam-database-authentication-for-rds-and-aurora) |
| Snowflake `CHANGES` is a net-effect window, not a log | A row updated three times inside one window yields one event; a row inserted and deleted inside it yields none. `Event::transaction` is always `None`, and there is no source order within a window | Inherent to the mechanism. The alternative — Snowflake Streams — requires a **write** to the source account to advance its offset, and commits that write before rustcdc's checkpoint, which turns a crash into silent at-most-once loss. Reasoning: [Snowflake source](@/library/snowflake.md#why-not-snowflake-streams) |
| The Snowflake transport is the embedder's | `SnowflakeSource` takes a `SnowflakeQueryExecutor` rather than opening its own connection | Snowflake speaks no wire protocol this crate could implement; reaching it needs HTTPS plus JWT/OAuth/WIF, which is a dependency tree the default build does not carry and — with no self-hostable server — could never be tested in CI. It also means every Snowflake authentication method works, including ones that do not exist yet. |
...
| Snowflake has no container-backed evidence | Every other connector is pinned against a real server in CI. Snowflake has no self-hostable implementation, so its statement text and result-set shape are unverified against a live account — [stated in full](@/library/snowflake.md#evidence-and-the-gap-in-it) |
...
[end-to-end runtime throughput](@/library/reliability-testing.md#end-to-end-runtime-throughput).
