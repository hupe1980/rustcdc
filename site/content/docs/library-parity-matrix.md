...
| Snapshot restricted to a subset of rows | A backfill is otherwise all-or-nothing, so restricting it to one tenant or one time range means not using it | Implemented — `IncrementalSnapshotConfig::with_table_condition` on all three connectors, Debezium's `additional-condition`. Bounds the chunk reads only; the live stream keeps carrying every change to the table | src/source/incremental_snapshot/driver.rs, tests/postgres_snapshot_filter_integration.rs, src/source/sqlserver/parser.rs (`a_row_filter_is_parenthesised_so_an_or_cannot_widen_the_keyset_seek`) |
