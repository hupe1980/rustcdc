//! Model-level checks of the crash-recovery *validator*, not of a real crash.
//!
//! Every test here drives [`CrashSimulationValidator`] over in-memory state. Nothing is
//! killed, no process boundary is crossed, and no database is involved. What these tests
//! establish is that the validator correctly classifies loss, duplication and checkpoint
//! divergence given a known event history — which is what makes it trustworthy when it is
//! pointed at a real crash.
//!
//! **The real crash tests are elsewhere**: `runtime_postgres_process_crash_integration.rs`,
//! `runtime_mysql_process_crash_integration.rs`, `runtime_mariadb_process_crash_integration.rs`
//! and `runtime_sqlserver_process_crash_integration.rs` spawn a child process against a live
//! database, kill it mid-stream, and assert on recovery from the checkpoint it left behind.
//!
//! This file was previously named `crash_simulation_integration.rs`, which read as though it
...
fn create_test_event(index: u64, table: &str) -> Event {
    Event::builder(table, Operation::Insert)
        .after(json!({"id": index, "name": format!("row_{}", index)}))
        .source(SourceMetadata::new(
            "test_source",
            format!("offset_{}", index),
            index * 1000,
        ))
        .ts(index * 1000)
        .schema("public")
...
fn create_postgres_stream_event(index: u64) -> Event {
    Event::builder("orders", Operation::Insert)
        .after(json!({"id": index, "payload": format!("pg-row-{index}")}))
        .source(SourceMetadata::new(
            "postgres",
            format!("0/{index:08X}"),
            index,
